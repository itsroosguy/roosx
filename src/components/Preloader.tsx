...
export const Preloader: React.FC<PreloaderProps> = ({ onComplete }) => {
  ...
  useEffect(() => {
    ...
    const DURATION = 2200; // 2.2 seconds flat
  ...
  const getPhaseText = (p: number) => {
    ...
    if (p < 65) return 'CALIBRATING BRAND ARCHITECTURE...';
    if (p < 90) return 'CRAFTING DIGITAL MOMENTUM...';
    ...
    <AnimatePresence mode="wait">
      {!isFinished && (
        <motion.div
          ...
          className="fixed inset-0 z-[99999] flex flex-col items-center justify-center bg-[#08080A] text-white select-none overflow-hidden"
        >
          {/* Subtle Ambient Radial Glow */}
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(255,122,26,0.18)_0%,transparent_65%)] pointer-events-none" />

          {/* Architectural Background Grid Mesh */}
          <div className="absolute inset-0 bg-[size:3.5rem_3.5rem] bg-[linear-gradient(to_right,#FF7A1A0a_1px,transparent_1px),linear-gradient(to_bottom,#FF7A1A0a_1px,transparent_1px)] [mask-image:radial-gradient(ellipse_70%_70%_at_50%_50%,#000_65%,transparent_100%)] pointer-events-none" />

          {/* Top Telemetry Header */}
          <div className="absolute top-6 left-6 sm:top-10 sm:left-10 flex items-center gap-3 pointer-events-none z-10">
          ...
          <div className="absolute top-6 right-6 sm:top-10 sm:right-10 text-right pointer-events-none z-10 hidden sm:block">
            <span className="font-mono text-[10px] text-zinc-400 tracking-widest uppercase font-semibold block">
              EST. 2026
            </span>
          </div>
          ...
          <div className="relative flex flex-col items-center justify-center z-10 px-4">
            ...
            <div className="relative w-56 h-56 sm:w-64 sm:h-64 flex items-center justify-center">
              ...
              {/* Spinning Dashed Telemetry Ring */}
              <motion.div
                animate={{ rotate: 360 }}
              ...
              {/* Progress Gauge SVG */}
              <svg className="w-full h-full transform -rotate-90 filter drop-shadow-[0_0_15px_rgba(255,122,26,0.4)]" viewBox="0 0 200 200">
                <defs>
              ...
              {/* Centered Drawn Kangaroo SVG Logo */}
            ...
            {/* Percentage Counter & Crisp Phase Status */}
            <div className="mt-6 flex flex-col items-center text-center space-y-2">
              <div className="font-display text-4xl sm:text-5xl font-extrabold tracking-tighter bg-gradient-to-r from-[#FF7A1A] via-[#FFA665] to-white bg-clip-text text-transparent">
